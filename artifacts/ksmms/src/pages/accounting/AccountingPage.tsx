...
const TYPE_LABELS: Record<string, string> = { disbursement: 'Disbursement', repayment: 'Repayment', interest_earned: 'Interest Earned', penalty: 'Penalty', write_off: 'Write-Off' };
const TYPE_COLORS: Record<string, string> = { disbursement: 'bg-red-100 text-red-800', repayment: 'bg-green-100 text-green-800', interest_earned: 'bg-blue-100 text-blue-800', penalty: 'bg-amber-100 text-amber-800', write_off: 'bg-gray-100 text-gray-800' };
const TRANSACTION_TYPES = [{ value: '', label: 'All Types' }, { value: 'disbursement', label: 'Disbursement' }, { value: 'repayment', label: 'Repayment' }, { value: 'interest_earned', label: 'Interest Earned' }, { value: 'penalty', label: 'Penalty' }, { value: 'write_off', label: 'Write-Off' }] as const;
...
export default function AccountingPage() {
  ...
  async function loadEntries() {
    try {
      const { data, error } = await api.get<AccountingEntry[]>('/accounting');
      if (error) throw new Error(error);
      setEntries(data || []);
    } catch { addNotification('error', 'Failed to load accounting entries'); }
    finally { setLoading(false); }
  }

  const filtered = useMemo(() => typeFilter ? entries.filter(e => e.transaction_type === typeFilter) : entries, [entries, typeFilter]);
  const stats = useMemo(() => {
    const sum = (type: string) => entries.filter(e => e.transaction_type === type).reduce((s, e) => s + e.amount, 0);
    return { disbursed: sum('disbursement'), collected: sum('repayment'), interest: sum('interest_earned'), outstanding: sum('disbursement') - sum('repayment') - sum('write_off') };
  }, [entries]);

  const columns = [
    { key: 'transaction_type', header: 'Type', render: (e: AccountingEntry) => <Badge colorClass={TYPE_COLORS[e.transaction_type]}>{TYPE_LABELS[e.transaction_type]}</Badge> },
  ...
  return (
    <div className="space-y-6">
      <div><h1 className="text-2xl font-bold text-gray-900">Accounting</h1><p className="text-sm text-gray-500 mt-1">Financial transactions and summary</p></div>
