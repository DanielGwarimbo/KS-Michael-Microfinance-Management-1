...
export default function LoanCreatePage() {
  ...
  useEffect(() => {
    recalculate();
  }, [form.principal, form.interest_rate, form.term_months, form.repayment_frequency]);

  async function loadClients() {
    const { data } = await supabase.from('clients').select('*').eq('status', 'active').order('first_name');
  ...
  function recalculate() {
    ...
    if (p > 0 && r >= 0 && t > 0) {
      setCalculation(calculateLoan(p, r, t, form.repayment_frequency as any));
    } else {
      setCalculation(null);
    }
  }
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    if (!calculation) {
      addNotification('error', 'Invalid loan parameters');
      return;
    }
            ...
            <div className="md:col-span-2">
              <Select
                ...
                options={[
                  ...
                  ...clients.map((c) => ({ value: c.id, label: `${c.client_number} - ${c.first_name} ${c.last_name}` })),
                ]}
                required
              />
            </div>
        ...
        {calculation && (
          <Card className="bg-teal-50 border-teal-200">
            ...
            <div className="grid grid-cols-3 gap-4">
              <div>
                <p className="text-sm text-teal-700">Total Payable</p>
                <p className="text-xl font-bold text-teal-900">{formatCurrency(calculation.totalPayable)}</p>
              </div>
              <div>
                <p className="text-sm text-teal-700">Total Interest</p>
                <p className="text-xl font-bold text-teal-900">{formatCurrency(calculation.totalInterest)}</p>
              </div>
              <div>
                <p className="text-sm text-teal-700">Installment Amount</p>
                <p className="text-xl font-bold text-teal-900">{formatCurrency(calculation.installmentAmount)}</p>
              </div>
            </div>
          </Card>
        )}
